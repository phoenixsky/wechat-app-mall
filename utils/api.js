...
// var API_BASE_URL_DEBUG = 'http://192.168.1.18:8080'
var API_BASE_URL_DEBUG = 'http://www.meetingplus.cn:8085'
var API_BASE_URL_RELEASE = 'https://www.meetingplus.cn'

var API_BASE_URL = IS_DEBUG ? API_BASE_URL_DEBUG : API_BASE_URL_RELEASE;
var subDomain = 'api/wshop'

//  图片前缀
var IMAGE_PREFIX_URL = (IS_DEBUG ? 'http://www.meetingplus.cn:8085' : API_BASE_URL_RELEASE) + '/uimg/'
...
const requestList = (url, needSubDomain, method, data,page) => {
  ...
  return new Promise((resolve,reject)=>{
    request(url, needSubDomain, method, {page,...data}).then(
      res => {
        if (res.code === 100 && res.data instanceof Array) {
          let data = res.data;
          if (data.length === 0) {
            let msg;
            if (page === 1) {
...
module.exports = {
  IS_DEBUG: IS_DEBUG,
  imgPre: IMAGE_PREFIX_URL,


  // 会管家登录接口
  loginHGJ: (wxCode) => {
    return request('/api/wxLogin', false, 'post', {
      wxCode
    })
  },
  ...
  // 觅会易登录接口
  login: (wxCode) => {
    return request('/api/wShopLogin', false, 'post', {
      wxCode
    })
  },
  //  觅会易注册接口
  register: ({
    ...
    return request('/api/register', false, 'post', {
      appId,
      code,
      iv,
      encryptedData
  ...
  // option
  getOptionList: (optionsName) => {
    return request('/api/options/getOptionsByName', false, 'post', {
      optionsName
    })
  },

  getUser: (userId) => {
    return request('/getUser', true, 'post', {
      userId
    })
  },
  ...
  // banner列表
  banners: () => {
    return request('/banner/list', true, 'post', )
  },

  // 商品列表
  goods: (data) => {
    return request('/skuList', true, 'post', data)
  },
  // 商品详情
  goodsDetail: (id) => {
    return request('/sku/' + id, true, 'post', {
      id
    })
  },
  // 提交订单
  addOrder: (order) => {
    return request('/addOrder', true, 'post', {
      orderDetail: JSON.stringify(order)
    })
  },
  orderList: (data,page) => {
    // return request('/orderList', true, 'get', data)
    return requestList('/orderList', true, 'get', data,page)
  },


  orderDetail: (orderId) => {
    return request('/order/' + orderId, true, 'get', {
      orderId
    })
  },
  cancelOrder: (orderId) => {
  ...
  wxPay: (orderId, openId) => {
    return request('/wxPay', true, 'post', {
      orderId,
      openId
    })
  },
  ...
  wxQRCode:(data)=>{
    return request('/wxPay', true, 'get', data)
  },
  ...
  modifyUserPassword: (token, pwdOld, pwdNew) => {
    return request('/user/modify/password', true, 'post', {
      token,
      pwdOld,
      pwdNew
    })
  ...
  goodsDynamic: (type) => {
    return request('/site/goods/dynamic', true, 'get', {
      type
    })
  },
  fetchSubDomainByWxappAppid: (appid) => {
    return request('/subdomain/appid/wxapp', false, 'get', {
      appid
    })
  },
